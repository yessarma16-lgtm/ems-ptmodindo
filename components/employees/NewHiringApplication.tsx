...
export function NewHiringApplication({ token, masterData, masterDataError, initialNewForm = false }: { token: string; masterData: EmployeeFormMasterData | null; masterDataError: string | null; initialNewForm?: boolean }) {
  ...
  if (newForm) return <><button type="button" className="mb-6 h-10 rounded-lg border border-input bg-card px-4 text-sm font-medium hover:bg-muted" onClick={() => setNewForm(false)}>Back</button><EmployeeForm mode="create" masterData={masterData} masterDataError={masterDataError} submitUrl={`/api/apply/new-hiring/${token}`} redirectTo="/apply/new-hiring/thanks" successMessage="Thank you for submitting your application!" submitLabel="Submit" excludeFields={EXCLUDED} sectionOrder={["Personal Information", "Employment Information", "Address & Contact", "Family Information", "Bank Information", "BPJS Information", "Tax Information", "Other Information"]} /></>;
  if (registration) return <EmployeeForm mode="edit" recordId={registration.recordId} initialValues={registration} masterData={masterData} masterDataError={masterDataError} submitUrl={`/api/new-hiring/lookup?record_id=${encodeURIComponent(registration.recordId)}`} redirectTo="/apply/new-hiring/thanks" successMessage="Application updated — advanced to the next review stage!" submitLabel="Resubmit" lockedFields={["name", "positionApplied"]} excludeFields={EXCLUDED} sectionOrder={["Personal Information", "Employment Information", "Address & Contact", "Family Information", "Bank Information", "BPJS Information", "Tax Information", "Other Information"]} />;
  return <form onSubmit={findApplicant} className="mx-auto max-w-md space-y-4 rounded-xl border bg-card p-6 shadow-sm"><label className="block text-sm font-medium">Login dengan No. KTP / Candidate No. / Application ID<input className="mt-2 h-10 w-full rounded-md border bg-background px-3" value={identifier} onChange={(e) => setIdentifier(e.target.value)} required /></label>{error && <p className="text-sm text-destructive">{error}</p>}<button className="h-10 w-full rounded-md bg-primary text-sm font-medium text-primary-foreground">Submit</button><a href={`/apply/new-hiring/${token}?new=1`} className="block text-center text-sm font-medium text-primary hover:underline">New Form</a></form>;
}
