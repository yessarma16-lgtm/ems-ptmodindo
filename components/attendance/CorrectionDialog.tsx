...
export function CorrectionDialog({ row, open, onOpenChange, onSaved }: CorrectionDialogProps) {
  ...
  async function submit() {
    ...
    try {
      const res = await fetch("/api/attendance/calculation/correct", {
        ...
        body: JSON.stringify({ id: row.id, newValue: parsedValue, note: note.trim() }),
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Koreksi Final OTH</DialogTitle>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Textarea id="correction-note" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Jelaskan alasan koreksi" />
          </div>
        </div>
        <DialogFooter>
